...
def unsharp_masking(img, sigma=10):
    '''Размыть изображение и вычесть из исходного, повышает резкость 
    (усиления контраста мелких деталей при неизменном общем контрасте)'''
...
def divide_masking(img, sigma=10):
    '''Альтернатива выравниванию гистограммы - изображение-частное (делим изображениеи на размытое)'''
    img_blured = gaussian_filter(img, sigma)
    img_filtered = img / img_blured
    return img_filtered 